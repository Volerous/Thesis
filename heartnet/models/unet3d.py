...
class UNet3D(Model):
    ...
    def __init__(self,
        ...
        self.receptive_field = compute_receptive_fields(
    ...
    def init_model(self):
        ...
        for i in range(self.depth):
            conv = Conv3D(int(filters * self.cf),
                          self.kernel_size,
        ...
        if self.flatten_output:
            out = Reshape([np.prod(self.img_shape[:3]), self.n_classes],
    ...
    def log(self):
        ...
        self.logger("Image dims:        (%i, %i, %i)" %
                    tuple(self.img_shape[:-1]))
        ...
        self.logger(
            "Crop:              %s" %
            (self.label_crop if np.sum(self.label_crop) != 0 else "None"))
